                                    ...
                                    <div class="old-price-list">
                                        @foreach ($recommended_product->stocks as $key => $stock)
                                            ...
                                            @if ($key == 0)
                                                @if ($offer != null)
                                                    <del
                                                        class="enable-del">AU ${{ formattedNepaliNumber($stock->price) }}</del>
                                                    {{-- @if ($stock->special_price != null)
                                                        <del>${{ $stock->special_price }}</del>
                                                    @endif --}}
                                                    <span class="price_list">AU ${{ formattedNepaliNumber($offer) }}</span>
                                                @elseif($stock->special_price)
                                                    <del
                                                        class="enable-del">AU ${{ formattedNepaliNumber($stock->price) }}</del>
                                                    <span
                                                        class="price_list">AU ${{ formattedNepaliNumber($stock->special_price) }}</span>
                                                @else
                                                    <span
                                                        class="price_list">AU ${{ formattedNepaliNumber($stock->price) }}</span>
                                                @endif
                                            @endif
                                        @endforeach
                                    </div>

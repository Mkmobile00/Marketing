                                            ...
                                            @foreach ($cart_item as $item)
                                                <tr>
                                                    ...
                                                    <td class="{{ $item->id }}" style="white-space: nowrap;">
                                                        <b>UR ${{ formattedNepaliNumber($item->sub_total_price) }}</b>
                                                    </td>
                                                    <td>
                            ...
                            <ul>
                                <li>
                                    ...
                                    <b class="cart_total_price">
                                        UR ${{ formattedNepaliNumber($cart->total_price + $cart->total_discount) }}</b>
                                </li>
                                <li>
                                    @php
                                    ...
                                    @endphp
                                    <span>Discount</span>
                                    <b class="total_discount_per">UR ${{ $cart->total_discount }}</b>
                                    {{-- <b class="total_discount_per">{{ $discount }} %</b> --}}
                                </li>
                                @if($vatAmount >0 && $vatAmount !=0)
                                <li>
                                    <span>Vat</span>
                                    <b class="sub_vat">UR ${{ formattedNepaliNumber($vatAmount) }}</b>
                                </li>
                                @endif
                                <li>
                                    <span>Sub Total</span>
                                    <b class="sub_total_price">UR ${{ formattedNepaliNumber($cart->total_price) }}</b>
                                </li>
                                @php
                                ...
                                <li>
                                    <span>Additional Charges</span>
                                    <b class="additional_charge">UR ${{ formattedNepaliNumber($totalAdditionalCharge) }}</b>
                                </li>
                                <li>
                                    <span>Total</span>
                                    <b class="sub_total_price">UR ${{ formattedNepaliNumber($cart->total_price + $totalAdditionalCharge) }}</b>
                                </li>
                            </ul>
                        </div>
                    @endif
                    ...
                    $('.Product-qty-change{{ $item->id }}').on('click', function(e) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                var para = '';
                                para += '<p class="' + item_id + '"><b>UR $' + response.data +
                                    '</b></p>';
                                // $('.cart-remove').replaceWith(response);
                                $('.total_cart_item').text('Item(' + response.cart_total_qty + ')');
                                $('.cart_total_price').text('UR $' + response.cart_total_price);
                                $('.total_discount_per').text(response.discount_per+' %');
                                $('.sub_vat').text('UR $'+response.vatAmount);
                                $('.sub_total_price').text('UR $'+response.sub_total_price);

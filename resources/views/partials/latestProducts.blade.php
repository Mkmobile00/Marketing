        ...
        <div class="bulk-section-content">
            ...
            @foreach ($new_arrival->stocks as $key => $stock)
                @if ($key == 0)
                    ...
                    @endphp
                    @if ($offer != null)
                        <del class="enable-del">AU ${{ formattedNepaliNumber($stock->price) }}</del>
                        <p class="price_list">AU ${{ formattedNepaliNumber($offer) }}</p>
                    @elseif($stock->special_price)
                        <del class="enable-del">AU ${{ formattedNepaliNumber($stock->price) }}</del>
                        <p class="price_list">AU ${{ formattedNepaliNumber($stock->special_price) }}</p>
                    @else
                        <p class="price_list">AU ${{ formattedNepaliNumber($stock->price) }}</p>
                    @endif
                @endif
            @endforeach
